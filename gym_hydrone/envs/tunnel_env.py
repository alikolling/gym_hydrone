...
class HydroneTunnelEnv(gym.Env):
    ...
    def _get_reward(self, observation):
        ...
        y_dist = np.linalg.norm(self.goal[1] - observation[1])
        ...
        reward_dist = max(
            ...
            - 0.05 * lin_vel_err
            - 0.01 * ang_vel_err
            - 0.05 * np.linalg.norm(self.action_base -
                                    observation[-4:]) / 1800,
        )
    ...
    def reset(self):
        ...
        self.goal = np.asarray([6.0, 0.0, 6.0])

        roll, pitch, yaw = euler_from_quaternion(
            self.initial_vehicle_orientation)
        self.goal_orientation[1] = math.pi / 6
