...
class HydroneCurriculumEnv(gym.Env):
    ...
    def _random_position(self):
        if self.env_stage == 0:
            return np.random.uniform(low=(-5.0, -5.0, 1.0),high=(5.0, 5.0, 5.))
        elif self.env_stage == 1:
            return np.random.uniform(low=(-5.0, -5.0, -4.5),high=(5.0, 5.0, 5.)) # hover in water or air
        elif self.env_stage == 2:
            return np.random.uniform(low=(-5.0, -5.0, -4.5), high=(5.0, 5.0, 5.))  # navigate to goal
    ...
    def _get_reward(self, observation):
        ...
        if self.env_stage == 0:
            terminated = dist > 0.5 # if too far, terminate
            success = dist < self.goalbox_distance
        ...
        elif self.env_stage in [2, 3]:
            success = dist < self.goalbox_distance
            return reward, terminated, success

    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        self.env_stage = options['stage']
